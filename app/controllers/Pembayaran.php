...
class Pembayaran extends Controller
{
    ...
    public function getPesanan($idPesanan)
    {
        ...
        $lapangan = $this->model("LapanganFutsal")->getLapanganById($pesanan["id_lapangan"]);
        $data["harga"] = $lapangan["harga"] * ($pesanan["jam_selesai"] - $pesanan["jam_mulai"]);
        $data["idPesanan"] = $idPesanan;
        $this->view("templates/header", $data);
        $this->view("pembayaran/index", $data);
        $this->view("templates/footer");
    ...
    public function uploadBukti()
    {
        ...
        if(isset($_POST["submit"]) && !empty($_FILES["fileBukti"]["name"])) {
            ...
            if(getimagesize($_FILES["fileBukti"]["tmp_name"])) {
                if (move_uploaded_file($_FILES["fileBukti"]["tmp_name"], $targetFilePath)) {
